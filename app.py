...
# ── ERCOT Auth Constants (from official ERCOT developer docs) ──────────────────
# These are the CORRECT values from ERCOT's public API documentation
TOKEN_URL  = "https://ercotb2c.b2clogin.com/ercotb2c.onmicrosoft.com/B2C_1_PUBAPI-ROPC-FLOW/oauth2/v2.0/token"
CLIENT_ID  = "fec253ea-0d06-4272-a5e6-941d3816b6cc"  # Official ERCOT public client ID
...
def get_token(username: str, password: str) -> str:
    ...
    if not resp.ok:
        raise Exception(f"Auth {resp.status_code}: {resp.json().get('error_description','')[:200]}")
    j = resp.json()
    return j.get("id_token") or j.get("access_token","")
...
def demo_hourly(node, for_date):
    ...
    h=pd.date_range(str(for_date),periods=24,freq="h")
    shape=np.array([5,4,3,3,4,8,14,18,16,14,13,14,16,15,14,16,22,38,48,42,28,18,12,8],dtype=float)
    return pd.DataFrame({"datetime":h,"lmp":np.maximum(-5,5+(s%20)+shape+np.random.normal(0,2.5,24)).round(2),"node":node})

def demo_hist(node, d_from, d_to, freq="D"):
    s=sum(ord(c) for c in node)%999; np.random.seed(s)
    dates=pd.date_range(d_from,d_to,freq=freq); n=len(dates); t=np.arange(n)
    lmp=np.maximum(-20,28+(s%35)+np.sin(t/(365 if freq=="D" else 12)*2*np.pi)*18
...
with st.sidebar:
    ...
    if sub_key and username and password:
        ...
        missing = [k for k,v in {"ERCOT_API_KEY":sub_key,"ERCOT_USERNAME":username,"ERCOT_PASSWORD":password}.items() if not v]
        st.error(f"Missing: {', '.join(missing)}")
        st.code('ERCOT_API_KEY  = "your-subscription-key"\nERCOT_USERNAME = "kartik.v@sunstripe.com"\nERCOT_PASSWORD = "yourPassword"', language="toml")
    ...
    today=date.today()
    if view=="📅 Today's LMP":
        sel_date=st.date_input("Date",value=today); d_from=d_to=str(sel_date); agg="Hourly"
    else:
        ...
        agg=st.selectbox("Aggregation",["Daily","Monthly","Yearly"],index=["Daily","Monthly","Yearly"].index(ad[preset]))
    ...
    for node in all_nodes:
        ...
        if df.empty:
            ...
            df=demo_hourly(node,date.fromisoformat(d_from)) if view=="📅 Today's LMP" \
               else demo_hist(node,d_from,d_to,"D" if agg=="Daily" else ("ME" if agg=="Monthly" else "YE"))
        dfs.append(df)

if using_demo:
    st.info("📊 **Demo data** shown. Add all 3 secrets to enable live ERCOT prices.")
...
with ch:
    ...
    for i,(node,df) in enumerate(zip(all_nodes,dfs)):
        color=COLORS[i%len(COLORS)]; adf=do_agg(df,agg).sort_values("datetime"); hb="lmp_max" in adf.columns
        if show_band and hb:
            fig.add_trace(go.Scatter(x=pd.concat([adf["datetime"],adf["datetime"][::-1]]),
                y=pd.concat([adf["lmp_max"],adf["lmp_min"][::-1]]),fill="toself",
                fillcolor=hex_rgba(color,0.08),line=dict(color="rgba(0,0,0,0)"),showlegend=False,hoverinfo="skip"))
    ...
    fig.update_layout(template="plotly_dark",paper_bgcolor="#0a0a14",plot_bgcolor="#111125",height=420,
...
if view!="📅 Today's LMP":
    st.markdown("---")
    comp=[{"Node":n,"Avg $/MWh":round(d["lmp"].mean(),2),"Max":round(d["lmp"].max(),2),"Min":round(d["lmp"].min(),2),"StdDev":round(d["lmp"].std(),2)} for n,d in zip(all_nodes,dfs) if not d.empty]
...
st.caption(f"ERCOT DAM Hourly LMP — NP4-183-CD | {'🟢 Live' if not using_demo else '🟡 Demo'}")
